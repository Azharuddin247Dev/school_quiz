...
let score = 0;
// studentName declared in firebase-config.js
let selectedClass = "";
let selectedSubject = "";
...
function startQuiz() {
  ...
  if (typeof questionBank === "undefined") {
    showMessage("প্রশ্ন লোড হচ্ছে না! পেজ রিফ্রেশ করুন।", 'error');
    return;
  }
...
function initializeQuestionSets() {
  ...
  for (let i = 0; i < allQuestions.length; i += 20) {
    ...
    if (setQuestions.length === 20) {
      // Only add complete sets of 20
      questionSets.push(setQuestions);
    }
  }

  console.log(
    `Total questions: ${allQuestions.length}, Total complete sets: ${questionSets.length}`
  );
  console.log(
    "Each question appears in exactly one set - no duplicates across sets"
  );
}
...
function prepareQuestions() {
  ...
  // Get user's current set index from Firebase or default
  if (!userProgress.currentSet && userProgress.currentSet !== 0) {
...
function updateSetIndicator() {
  ...
  const setNumber = (currentSetIndex + 1)
    .toString()
    .split("")
    .map((d) => bengaliNumbers[parseInt(d)])
    .join("");
...
function updateProgress() {
  ...
  document.getElementById(
    "question-counter"
  ).textContent = `${currentNum} / ${totalNum}`;
}
...
function showResults() {
  ...
  if (isAuthenticated) {
    saveScoreToLeaderboard(score, totalQuestions, currentSetIndex);
  }

  // Update user progress in Firebase
  if (isAuthenticated && currentUser) {
    updateUserProgress(currentSetIndex);
  }
  ...
  const bengaliPercentage = percentage
    .toString()
    .split("")
    .map((d) => {
      const bengaliNumbers = ["০", "১", "২", "৩", "৪", "৫", "৬", "৭", "৮", "৯"];
  ...
  document.getElementById(
    "student-info"
  ).textContent = `অভিনন্দন, ${studentName}!`;

  const bengaliScore = score
    .toString()
    .split("")
    .map((d) => {
      const bengaliNumbers = ["০", "১", "২", "৩", "৪", "৫", "৬", "৭", "৮", "৯"];
  ...
  const bengaliTotal = totalQuestions
    .toString()
    .split("")
    .map((d) => {
      const bengaliNumbers = ["০", "১", "২", "৩", "৪", "৫", "৬", "৭", "৮", "৯"];
  ...
  const currentSetNum = (currentSetIndex + 1)
    .toString()
    .split("")
    .map((d) => {
      const bengaliNumbers = ["০", "১", "২", "৩", "৪", "৫", "৬", "৭", "৮", "৯"];
  ...
  document.getElementById(
    "score-details"
  ).textContent = `সেট ${currentSetNum}/${totalSets}: আপনি ${bengaliTotal}টি প্রশ্নের মধ্যে ${bengaliScore}টি সঠিক উত্তর দিয়েছেন`;
...
function showNextSetOption() {
  const nextSetIndex = userProgress.currentSet || 0;

  let nextSetHtml = "";

  if (nextSetIndex < questionSets.length) {
    // More sets available
    const nextSetNum = (nextSetIndex + 1)
      .toString()
      .split("")
      .map((d) => {
...
function showSubjectBreakdown() {
  ...
  const subjectNamesMap =
    typeof subjectNames !== "undefined" ? subjectNames : localSubjectNames;
      ...
      html += `
                <div class="subject-score">
                    <span class="subject-name">${
                      subjectNamesMap[subject] || subject
                    }:</span>
                    <span class="subject-result">${subjectData.correct}/${
        subjectData.total
      } (${subjectPercentage}%)</span>
                </div>
            `;
    }
  });
              ...
              answer.isCorrect ? "correct-answer" : "incorrect-answer"
                ...
                <div class="answer-header">
                    ...
                    <span class="subject-tag">${
                      (typeof subjectNames !== "undefined"
                        ? subjectNames[answer.subject]
                        : answer.subject) || "অজানা বিষয়"
                    }</span>
                </div>
                ...
                <div class="answer-options">
                    ${answer.options
                      .map(
                        ...
                        } ${i === answer.userAnswer ? "user-option" : ""}">
                            ${option} ${
                          i === answer.correctAnswer ? "✓" : ""
                        } ${
                          i === answer.userAnswer && i !== answer.correctAnswer
                            ? "✗"
                            : ""
                        }
                        </div>
...
// Removed local tracking - using Firebase global leaderboard

// Show global leaderboard (double-click footer)
        ...
        <div class="popup-overlay">
            <div class="popup-content">
                ...
                <div class="popup-body">
                    ${
                      players.length === 0
                        ? '<p class="no-players">কেউ এখনো খেলেনি!</p>'
                        : players
                            .map(
                              (p, i) => `
...
const englishWords = [
  {
    word: "COMPUTER",
    clue: "Electronic device for processing data",
    blank: "C_MP_T_R",
  },
...
function nextChemistryElement() {
  ...
  document.getElementById("chemistry-element").textContent =
    currentElement.name;
...
function showStartScreen() {
  ...
  if (nameInput) {
    ...
    nameInput.disabled = true; // Disable editing since it comes from auth
  }
...
function loadCompletedSets() {
  if (!userProgress || !userProgress.currentSet || userProgress.currentSet === 0) {
  ...
  for (let i = 0; i < userProgress.currentSet; i++) {
    const setNumber = (i + 1)
      .toString()
      .split("")
      .map((d) => bengaliNumbers[parseInt(d)])
...
function startSelectedSet() {
  const selectedSetIndex = parseInt(
    document.getElementById("completed-sets").value
  );
