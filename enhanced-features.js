...
function getUserStats() {
    const userScores = globalLeaderboard.filter(s => s.userId === currentUser.uid);
    ...
    // Calculate rank
    const allUsers = [...new Set(globalLeaderboard.map(s => s.userId))];
    const userBestScores = allUsers.map(userId => {
        const userScores = globalLeaderboard.filter(s => s.userId === userId);
        return Math.max(...userScores.map(s => s.percentage));
    }).sort((a, b) => b - a);
    
    const rank = userBestScores.indexOf(bestScore) + 1;
